...
territory_visits = visits_df.groupby("Territory Code").size().reset_index(name="Visit Count")
...
pharmacy_counts = visits_df.groupby('Pharmacies').agg({'Visit Date': 'count', 'Territory Code': 'first'}).reset_index()
...
territory_density = visits_df.groupby('Territory Code')['Pharmacies'].nunique().reset_index(name='Pharmacy Count')
...
rep_data = visits_df.groupby('Rep').agg(Total_Visits=('Visit Date', 'count'), Unique_Pharmacies=('Pharmacies', pd.Series.nunique), Territories=('Territory Code', pd.Series.nunique)).reset_index()
...
low_readiness_territories = ['K105', 'K406']  # from earlier analysis
priority_zones = pharmacy_counts[pharmacy_counts['Territory Code'].isin(low_readiness_territories)]
